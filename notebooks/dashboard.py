...
SILVER_PATH = "/Users/krithikaannaswamykannan/fraud-pipeline/warehouse/fraud/silver_transactions/data/*.parquet"
...
if df is not None:
    ...
    avg_amount = df["TransactionAmt"].mean()
    fraud_avg = df[df["isFraud"]==1]["TransactionAmt"].mean()
else:
...
with c5:
    st.markdown(f"<div class=\"metric-card\"><div class=\"metric-label\">Model Accuracy</div><div class=\"metric-number\">{round(best_auc*100,2)}%</div><div class=\"metric-delta delta-up\">Detection Accuracy</div></div>", unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)
tab1, tab2, tab3 = st.tabs(["Analytics", "Fraud Patterns", "Score Transaction"])
...
with tab1:
    if df is not None:
        ...
        with col_left:
            ...
            fraud_amounts = df[df["isFraud"]==1]["TransactionAmt"].clip(upper=500)
            legit_amounts = df[df["isFraud"]==0]["TransactionAmt"].clip(upper=500).sample(min(1000, legit_count))
            fig = go.Figure()
        ...
        with col_right:
            ...
            card_data = df.groupby("card4")["isFraud"].agg(["sum","count"]).reset_index()
            card_data.columns = ["Network","Fraud","Total"]
            card_data["Rate"] = (card_data["Fraud"] / card_data["Total"] * 100).round(2)
            card_data = card_data[card_data["Total"] > 5].sort_values("Rate", ascending=True)
            fig2 = go.Figure(go.Bar(x=card_data["Rate"], y=card_data["Network"], orientation="h", marker=dict(color=card_data["Rate"], colorscale=[[0,"#1e3a5f"],[0.5,"#7c3aed"],[1.0,"#ef4444"]]), text=[f"{r:.1f}%" for r in card_data["Rate"]], textposition="outside", textfont=dict(color="#94a3b8", size=11)))
        ...
        df_copy = df.copy()
        df_copy["tier"] = pd.cut(df_copy["TransactionAmt"], bins=bins, labels=labels)
        tier_data = df_copy.groupby(["tier","risk_label"]).size().reset_index(name="count")
        fig3 = px.bar(tier_data, x="tier", y="count", color="risk_label", color_discrete_map={"FRAUD":"#f87171","LEGIT":"#34d399"}, barmode="stack")
...
with tab2:
    if df is not None:
        ...
        with col_a:
            st.markdown("<div class=\"section-header\">Flagged Transactions by Channel</div>", unsafe_allow_html=True)
            email_fraud = df[df["isFraud"]==1].groupby("P_emaildomain").size().reset_index(name="count")
            email_fraud = email_fraud.sort_values("count", ascending=False).head(12)
            email_fraud["domain"] = email_fraud["P_emaildomain"].fillna("unknown")
            fig4 = px.treemap(email_fraud, path=["domain"], values="count", color="count", color_continuous_scale=[[0,"#1e1b4b"],[0.5,"#7c3aed"],[1.0,"#ef4444"]])
        ...
        with col_b:
            ...
            card6_fraud = df.groupby("card6").agg(total=("isFraud","count"), fraud=("isFraud","sum")).reset_index()
        ...
        top_emails = df[df["isFraud"]==1]["P_emaildomain"].value_counts().head(8).index.tolist()
        hm_df = df[df["P_emaildomain"].isin(top_emails)]
        ...
        flagged = df[df["isFraud"]==1][["TransactionID","TransactionAmt","card4","card6","P_emaildomain","risk_label"]].sort_values("TransactionAmt", ascending=False).head(20)
        st.dataframe(flagged.rename(columns={"TransactionID":"Transaction ID","TransactionAmt":"Amount ($)","card4":"Card Network","card6":"Card Type","P_emaildomain":"Channel","risk_label":"Status"}), use_container_width=True, hide_index=True)
